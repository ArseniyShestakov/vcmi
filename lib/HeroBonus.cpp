...
BonusList::BonusList(bool BelongsToTree /* =false */) :
...
{
	if(hasBonusOfType(Bonus::NON_LIVING) || hasBonusOfType(Bonus::UNDEAD) || hasBonusOfType(Bonus::NO_MORALE) || hasBonusOfType(Bonus::SIEGE_WEAPON))
		return 0;
...
void CBonusSystemNode::limitBonuses(const BonusList & allBonuses, BonusList & out) const
{
	...
	while(true)
	{
		...
		{
			auto b = undecided[i];
			BonusLimitationContext context =
			{
				b, *this, out
			};
...
namespace Selector
{
	...
	DLL_LINKAGE CSelector all([](const Bonus * b)
	{
		return true;
	});
	DLL_LINKAGE CSelector none([](const Bonus * b)
	{
		return false;
	});
