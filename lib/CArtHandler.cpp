...
std::vector<JsonNode> CArtHandler::loadLegacyData(size_t dataSize)
{
	...
	#define ART_POS(x) #x,
	const std::vector<std::string> artSlots =
	{
		ART_POS_LIST
	};
	#undef ART_POS
...
#define ART_POS(x) { #x, ArtifactPosition::x },
	static const std::map<std::string, ArtifactPosition> artifactPositionMap =
	{
		ART_POS_LIST
	};
#undef ART_POS
...
#define ART_BEARER(x) { #x, ArtBearer::x },
	static const std::map<std::string, int> artifactBearerMap =
	{
		ART_BEARER_LIST
	};
#undef ART_BEARER
...
{
	return pickRandomArtifact(rand, flags, [](ArtifactID)
	{
		return true;
	});
}
...
bool CArtHandler::legalArtifact(ArtifactID id)
{
	...
	return ((art->possibleSlots[ArtBearer::HERO].size() || (art->possibleSlots[ArtBearer::COMMANDER].size() && VLC->modh->modules.COMMANDERS) || (art->possibleSlots[ArtBearer::CREATURE].size() && VLC->modh->modules.STACK_ARTIFACT)) && !(art->constituents) && //no combo artifacts spawning
		art->aClass >= CArtifact::ART_TREASURE && art->aClass <= CArtifact::ART_RELIC);
}
...
bool CArtifactSet::hasArt(ui32 aid, bool onlyWorn /*= false*/, bool searchBackpackAssemblies /*= false*/) const
{
	return getArtPos(aid, onlyWorn) != ArtifactPosition::PRE_FIRST || (searchBackpackAssemblies && getHiddenArt(aid));
}
