...
void CGTeleport::addToChannel(std::map<TeleportChannelID, std::shared_ptr<TeleportChannel>> & channelsList, const CGTeleport * obj)
{
	...
	if(tc->entrances.size() && tc->exits.size() && (tc->entrances.size() != 1 || tc->entrances != tc->exits))
	{
...
{
	if(h->hasBonusOfType(Bonus::WHIRLPOOL_PROTECTION) || (h->stacksCount() == 1 && h->Slots().begin()->second->count == 1)) //we can't remove last unit
	{
		return true;
	}
	return false;
...
void CGObservatory::onHeroVisit(const CGHeroInstance * h) const
{
	...
	case Obj::COVER_OF_DARKNESS:
	{
		...
		{
			if(cb->getPlayerStatus(player.first) == EPlayerStatus::INGAME && cb->getPlayerRelations(player.first, h->tempOwner) == PlayerRelations::ENEMIES)
