...
{
	bool intersect = right() > rect.left() && rect.right() > left() && bottom() > rect.top() && rect.bottom() > top() && z == rect.z;
	if(intersect)
	{
		MapRect ret;
...
TerrainViewPattern::WeightedRule::WeightedRule(std::string & Name) :
	points(0), name(Name)
{
	standardRule = (TerrainViewPattern::RULE_ANY == Name || TerrainViewPattern::RULE_DIRT == Name || TerrainViewPattern::RULE_NATIVE == Name || TerrainViewPattern::RULE_SAND == Name || TerrainViewPattern::RULE_TRANSITION == Name || TerrainViewPattern::RULE_NATIVE_STRONG == Name);
...
CTerrainViewPatternConfig::CTerrainViewPatternConfig()
{
	...
	static const std::string patternTypes[] =
	{
		"terrainView", "terrainType"
	};
		...
		if(tiles.nativeTiles.find(centerPos) != tiles.nativeTiles.end())
		{
			...
			rect.forEach([&](const int3 & posToTest)
			{
				...
				if(centerTile.terType != terrainTile.terType)
				{
					...
					{
						if((nativeTilesCntNorm > invalidNativeTilesCnt) || (nativeTilesCntNorm == invalidNativeTilesCnt && testTile.foreignTiles.size() < invalidForeignTilesCnt))
						{
							putSuitableTile = true;
						}
...
CDrawTerrainOperation::ValidationResult CDrawTerrainOperation::validateTerrainViewInner(const int3 & pos, const TerrainViewPattern & pattern, int recDepth /*= 0*/) const
{
	...
	for(int i = 0; i < 9; ++i)
	{
		...
		for(auto & elem : pattern.data[i])
		{
			...
			{
				bool dirtTestOk = (rule.isDirtRule() || rule.isTransition()) && isAlien && !isSandType(terType);
				bool sandTestOk = (rule.isSandRule() || rule.isTransition()) && isSandType(terType);

				if(transitionReplacement.empty() && rule.isTransition()	&& (dirtTestOk || sandTestOk))
				{
					transitionReplacement = dirtTestOk ? TerrainViewPattern::RULE_DIRT : TerrainViewPattern::RULE_SAND;
				}
				if(rule.isTransition())
				{
					applyValidationRslt((dirtTestOk && transitionReplacement != TerrainViewPattern::RULE_SAND) || (sandTestOk && transitionReplacement != TerrainViewPattern::RULE_DIRT));
				}
				else
				{
			...
			{
				bool sandTestOk = (rule.isSandRule() || rule.isTransition()) && isAlien;
