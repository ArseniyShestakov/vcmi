...
static const int3 dirs4[] =
{
	int3(0, 1, 0), int3(0, -1, 0), int3(-1, 0, 0), int3(+1, 0, 0)
};
static const int3 dirsDiagonal[] =
{
...
std::string CMapGenerator::getMapDescription() const
{
	...
	const std::string waterContentStr[3] =
	{
		"none", "normal", "islands"
	};
	const std::string monsterStrengthStr[3] =
	{
		"weak", "normal", "strong"
	};
		...
		if(posA.z != posB.z) //try to place subterranean gates
		{
			...
			while(!guardPos.valid())
			{
				...
				for(auto tile : commonTiles)
				{
					...
					{
						if(zoneA->areAllTilesAvailable(this, gate1, tile, tilesBlockedByObject) && zoneB->areAllTilesAvailable(this, gate2, otherTile, tilesBlockedByObject))
						{
