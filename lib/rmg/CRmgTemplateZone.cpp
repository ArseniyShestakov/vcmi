...
{
	assert(value.find(ETerrainType::WRONG) == value.end() && value.find(ETerrainType::BORDER) == value.end() && value.find(ETerrainType::WATER) == value.end() && value.find(ETerrainType::ROCK) == value.end());
	terrainTypes = value;
}
...
bool CRmgTemplateZone::addMonster(CMapGenerator * gen, int3 & pos, si32 strength, bool clearSurroundingTiles, bool zoneGuard)
{
	...
	{
		7500, 7500, 7500, 5000, 5000
	};
	static const float multiplier1[] =
	{
		0.5, 0.75, 1.0, 1.5, 1.5
	};
	static const float multiplier2[] =
	{
...
{
	static const Res::ERes woodOre[] =
	{
		Res::ERes::WOOD, Res::ERes::ORE
	};
	static const Res::ERes preciousResources[] =
	{
		Res::ERes::GEMS, Res::ERes::CRYSTAL, Res::ERes::MERCURY, Res::ERes::SULFUR
	};
...
void CRmgTemplateZone::createTreasures(CMapGenerator * gen)
{
	...
	static int minGuardedValues[] =
	{
		6500, 4167, 3000, 1833, 1333
	};
...
void CRmgTemplateZone::connectRoads(CMapGenerator * gen)
{
	...
	while(!roadNodesCopy.empty())
	{
		...
		auto comparator = [=](int3 lhs, int3 rhs)
			{
				return node.dist2dSQ(lhs) < node.dist2dSQ(rhs);
			};
...
void CRmgTemplateZone::addAllPossibleObjects(CMapGenerator * gen)
{
	...
	//prisons
	//levels 1, 5, 10, 20, 30
	static int prisonExp[] =
	{
		0, 5000, 15000, 90000, 500000
	};
	static int prisonValues[] =
	{
	...
	static int elementalConfluxROE[] =
	{
		7, 13, 16, 47
	};
	for(int i = 0; i < 4; i++)
	...
	if(questArtZone) //we won't be placing seer huts if there is no zone left to place arties
	{
		...
		static int seerExpGold[] =
		{
			5000, 10000, 15000, 20000
		};
		static int seerValues[] =
		{
			2000, 5333, 8666, 12000
		};
