...
float FuzzyHelper::getTacticalAdvantage(const CArmedInstance * we, const CArmedInstance * enemy)
{
	...
	{
		fl::InputVariable * tab[] =
		{
			ta.bankPresent, ta.castleWalls, ta.ourWalkers, ta.ourShooters, ta.ourFlyers, ta.ourSpeed, ta.enemyWalkers, ta.enemyShooters, ta.enemyFlyers, ta.enemySpeed
		};
		std::string names[] =
		{
			"bankPresent", "castleWalls", "ourWalkers", "ourShooters", "ourFlyers", "ourSpeed", "enemyWalkers", "enemyShooters", "enemyFlyers", "enemySpeed"
		};
...
void FuzzyHelper::initVisitTile()
{
	try
	{
		...
		std::vector<fl::InputVariable *> helper =
		{
			vt.strengthRatio, vt.heroStrength, vt.turnDistance, vt.missionImportance
		};
		for(auto val : helper)
		{
