...
//LOG_TRACE_PARAMS(logAi, "askID '%i', exits '%s'", askID % exits);
	...
	for(auto exit : exits)
	{
		...
		else
		{
			...
			if(obj == nullptr && !vstd::contains(teleportChannelProbingList, exit.first) &&	exit.first != destinationTeleport)
			{
...
void VCAI::performObjectInteraction(const CGObjectInstance * obj, HeroPtr h)
{
	...
	case Obj::TOWN:
		...
		if(h->visitedTown) //we are inside, not just attacking
		{
			...
			if(!h->hasSpellbook() && cb->getResourceAmount(Res::GOLD) >= GameConstants::SPELLBOOK_GOLD_COST + saving[Res::GOLD] && h->visitedTown->hasBuilt(BuildingID::MAGES_GUILD_1))
...
static const BuildingID essential[] =
{
	BuildingID::TAVERN, BuildingID::TOWN_HALL
};
static const BuildingID goldSource[] =
{
...
bool VCAI::isGoodForVisit(const CGObjectInstance * obj, HeroPtr h, SectorMap & sm)
{
	...
	if(!targetPos.valid())
		return false;
	if(isTileNotReserved(h.get(), targetPos) && !obj->wasVisited(playerID) && (cb->getPlayerRelations(ai->playerID, obj->tempOwner) == PlayerRelations::ENEMIES || isWeeklyRevisitable(obj)) && //flag or get weekly resources / creatures
	   isSafeToVisit(h, pos) && shouldVisit(h, obj) && !vstd::contains(alreadyVisited, obj) && !vstd::contains(reservedObjs, obj) && isAccessibleForHero(targetPos, h))
	{
...
bool VCAI::canRecruitAnyHero(const CGTownInstance * t) const
{
	...
	if(t)
		return cb->getResourceAmount(Res::GOLD) >= GameConstants::HERO_GOLD_COST && cb->getHeroesInfo().size() < ALLOWED_ROAMING_HEROES && cb->getAvailableHeroes(t).size();
	else
		return false;
}
...
bool VCAI::isAccessibleForHero(const int3 & pos, HeroPtr h, bool includeAllies /*= false*/) const
{
	...
	{ //don't visit tile occupied by allied hero
		...
		{
			if(obj->ID == Obj::HERO && cb->getPlayerRelations(ai->playerID, obj->tempOwner) != PlayerRelations::ENEMIES && obj != h.get())
				return false;
		}
	}
...
bool VCAI::moveHeroToTile(int3 dst, HeroPtr h)
{
	...
	else
	{
		...
		auto isTeleportAction = [&](CGPathNode::ENodeAction action) -> bool
			{
				if(action != CGPathNode::TELEPORT_NORMAL && action != CGPathNode::TELEPORT_BLOCKING_VISIT && action != CGPathNode::TELEPORT_BATTLE)
				{
					return false;
				}
		...
		for(; i > 0; i--)
		{
			...
			if((i - 2 >= 0) //Check there is node after next one; otherwise transit is pointless
			   && (CGTeleport::isConnected(nextObjectTop, getObj(path.nodes[i - 2].coord, false)) || CGTeleport::isTeleport(nextObjectTop)))
...
void VCAI::tryRealize(Goals::Build & g)
{
	for(const CGTownInstance * t : cb->getTownsInfo())
	{
		...
		if(!ai->primaryHero() || (t->getArmyStrength() > ai->primaryHero()->getArmyStrength() * 2 && !isAccessibleForHero(t->visitablePos(), ai->primaryHero())))
		{
			recruitHero(t);
			buildArmyIn(t);
		}
...
{ //TODO: allow polling of remaining creatures in dwelling
	if(dynamic_cast<const CGVisitableOPW *>(obj) || //ensures future compatibility, unlike IDs
	   dynamic_cast<const CGDwelling *>(obj) || dynamic_cast<const CBank *>(obj)) //banks tend to respawn often in mods
		return true;
	switch(obj->ID)
	{
	case Obj::STABLES:
		...
		if(preds[dest])
		{
			...
			if(!src->water && sectorToReach->water) //embark
			{
				...
				auto firstEP = boost::find_if(src->embarkmentPoints, [=](crint3 pos) -> bool
				{
					...
					return t && t->visitableObjects.size() == 1 && t->topVisitableId() == Obj::BOAT	&& retreiveTile(pos) == sectorToReach->id;
				});
...
int3 SectorMap::findFirstVisitableTile(HeroPtr h, crint3 dst)
{
	...
	while(curtile != h->visitablePos())
	{
		...
		if(topObj && topObj->ID == Obj::HERO && topObj != h.h && cb->getPlayerRelations(h->tempOwner, topObj->tempOwner) != PlayerRelations::ENEMIES)
		{
