...
bool canBeEmbarkmentPoint(const TerrainTile * t, bool fromWater)
{
	...
	return !t->blocked || (!fromWater && t->visitableObjects.size() == 1 && t->topVisitableId() == Obj::BOAT);
...
{
	return cb->getTile(tileToHit)->topVisitableId() == Obj::BORDER_GATE && (dynamic_cast<const CGKeys *>(cb->getTile(tileToHit)->visitableObjects.back()))->wasMyColorVisited(ai->playerID);
}
