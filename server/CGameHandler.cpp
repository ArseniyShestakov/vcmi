...
void CGameHandler::levelUpCommander(const CCommanderInstance * c)
{
	...
	{
		if(c->secondarySkills.at(specialSkill.second.first) == ECommander::MAX_SKILL_LEVEL && c->secondarySkills.at(specialSkill.second.second) == ECommander::MAX_SKILL_LEVEL && !vstd::contains(c->specialSKills, i))
			clu.skills.push_back(i);
		++i;
	}
	...
	if(result == BattleResult::NORMAL && finishingBattle->winnerHero)
	{
		...
		if(finishingBattle->loserHero)
		{
			...
			for(auto artSlot : artifactsWorn)
			{
				...
				if(art && !art->artType->isBig() && art->artType->id != ArtifactID::SPELLBOOK)
...
void CGameHandler::battleAfterLevelUp(const BattleResult & result)
{
	...
	{
		finishingBattle->loser, finishingBattle->victor
	};
...
void CGameHandler::prepareAttack(BattleAttack & bat, const CStack * att, const CStack * def, int distance, int targetHex)
{
	...
	auto sideHeroBlocksLuck = [](const SideInBattle & side)
		{
			return NBonus::hasOfType(side.hero, Bonus::BLOCK_LUCK);
		};
	...
	{
		static const int artilleryLvlToChance[] =
		{
			0, 50, 75, 100
		};
...
void CGameHandler::applyBattleEffects(BattleAttack & bat, const CStack * att, const CStack * def, int distance, bool secondary) //helper function for prepareAttack
{
	...
	if(att->hasBonusOfType(Bonus::SOUL_STEAL) && def->isLiving())
	{
		...
		if(std::any_of(shi.healedStacks.begin(), shi.healedStacks.end(), [](StacksHealedOrResurrected::HealInfo healInfo)
		{
			return healInfo.healedHP > 0;
		}))
		{
			bsa.healedStacks.push_back(shi);
		}
	...
	if(!bat.shot() && !vstd::contains(def->state, EBattleStackState::CLONED) && def->hasBonusOfType(Bonus::FIRE_SHIELD) && !att->hasBonusOfType(Bonus::FIRE_IMMUNITY))
	{
...
int CGameHandler::moveStack(int stack, BattleHex dest)
{
	...
	if(battleGetSiegeLevel() > 0 && curStack->side == BattleSide::DEFENDER && dbState != EGateState::DESTROYED && dbState != EGateState::BLOCKED)
	{
		canUseGate = true;
	}
	...
	if(curStack->hasBonusOfType(Bonus::FLYING))
	{
		...
		{
			if(canUseGate && dbState != EGateState::OPENED && occupyGateDrawbridgeHex(dest))
			{
			...
			for(int i = path.first.size() - 1; i >= 0; i--)
			{
				...
				if(!gateMayCloseAtHex.isValid() && dbState != EGateState::CLOSED)
				{
					...
					if(gs->curB->town->subID == ETownType::FORTRESS)
					{
						...
						else if(hex == ESiegeHex::GATE_OUTER && i - 1 >= 0 && path.first[i - 1] != ESiegeHex::GATE_INNER && path.first[i - 1] != ESiegeHex::GATE_BRIDGE)
						{
							gateMayCloseAtHex = path.first[i - 1];
						}
					}
		...
		while(stackIsMoving)
		{
			...
			else
			{
				for(bool obstacleHit = false; (!obstacleHit) && (!gateStateChanging) && (v >= tilesToMove); --v)
				{
					...
					if((openGateAtHex.isValid() && openGateAtHex == hex) ||	(gateMayCloseAtHex.isValid() && gateMayCloseAtHex == hex))
					{
						gateStateChanging = true;
					}
...
void CGameHandler::newTurn()
{
	...
	for(CGTownInstance * t : gs->map->towns)
	{
		...
		if(t->hasBonusOfType(Bonus::DARKNESS))
		{
			...
			{
				if(getPlayerStatus(player.first) == EPlayerStatus::INGAME && getPlayerRelations(player.first, t->tempOwner) == PlayerRelations::ENEMIES)
...
bool CGameHandler::moveHero(ObjectInstanceID hid, int3 dst, ui8 teleporting, bool transit, PlayerColor asker /*= 255*/)
{
	...
	if(((t.terType == ETerrainType::ROCK || (t.blocked && !t.visitable && !canFly))	&& complain("Cannot move hero, destination tile is blocked!")) || ((!h->boat && !canWalkOnSea && !canFly && t.terType == ETerrainType::WATER && (t.visitableObjects.size() < 1 || (t.visitableObjects.back()->ID != Obj::BOAT && t.visitableObjects.back()->ID != Obj::HERO))) //hero is not on boat/water walking and dst water tile doesn't contain boat/hero (objs visitable from land) -> we test back cause boat may be on top of another object (#276)
																			   && complain("Cannot move hero, destination tile is on water!")) || ((h->boat && t.terType != ETerrainType::WATER && t.blocked) && complain("Cannot disembark hero, tile is blocked!")) || ((distance(h->pos, dst) >= 1.5 && !teleporting) && complain("Tiles are not neighboring!"))	|| ((h->inTownGarrison)	&& complain("Can not move garrisoned hero!")) || ((h->movement < cost && dst != h->pos && !teleporting)	&& complain("Hero doesn't have any movement points left!")) || ((transit && !canFly && !CGTeleport::isTeleport(t.topVisitableObj())) && complain("Hero cannot transit over this tile!"))
...
bool CGameHandler::teleportHero(ObjectInstanceID hid, ObjectInstanceID dstid, ui8 source, PlayerColor asker /* = 255*/)
{
	...
	if(((h->getOwner() != t->getOwner()) && complain("Cannot teleport hero to another player")) || ((!from || !from->hasBuilt(BuildingID::CASTLE_GATE, ETownType::INFERNO))	&& complain("Hero must be in town with Castle gate for teleporting")) || (!t->hasBuilt(BuildingID::CASTLE_GATE, ETownType::INFERNO) && complain("Cannot teleport hero to town without Castle gate in it")))
		return false;
...
bool CGameHandler::arrangeStacks(ObjectInstanceID id1, ObjectInstanceID id2, ui8 what, SlotID p1, SlotID p2, si32 val, PlayerColor player)
{
	...
	if(what == 1) //swap
	{
		if(((s1->tempOwner != player && s1->tempOwner != PlayerColor::UNFLAGGABLE) && s1->getStackCount(p1)) || ((s2->tempOwner != player && s2->tempOwner != PlayerColor::UNFLAGGABLE) && s2->getStackCount(p2)))
		{
	...
	{
		if((s1->getCreature(p1) != s2->getCreature(p2) && complain("Cannot merge different creatures stacks!"))	|| (((s1->tempOwner != player && s1->tempOwner != PlayerColor::UNFLAGGABLE) && s2->getStackCount(p2)) && complain("Can't take troops from another player!")))
			return false;
	...
	else if(what == 3) //split
	{
		...
		if((s1->tempOwner != player && countLeftOnSrc < s1->getStackCount(p1)) || (s2->tempOwner != player && val < s2->getStackCount(p2)))
		{
		...
		if((!vstd::contains(S1.stacks, p1) && complain("no creatures to split")) || (val < 1 && complain("no creatures to split")))
		{
			return false;
		}
		...
		if(vstd::contains(S2.stacks, p2)) //dest. slot not free - it must be "rebalancing"...
		{
			...
			if((total < val && complain("Cannot split that stack, not enough creatures!")) || (s1->getCreature(p1) != s2->getCreature(p2) && complain("Cannot rebalance different creatures stacks!")))
			{
				return false;
			}
	...
	auto processAfterBuiltStructure = [t, this](const BuildingID buildingID)
		{
			if(buildingID <= BuildingID::MAGES_GUILD_5 || //it's mage guild
			   (t->subID == ETownType::TOWER && buildingID == BuildingID::LIBRARY) || (t->subID == ETownType::CONFLUX && buildingID == BuildingID::GRAIL))
			{
				if(t->visitingHero)
...
bool CGameHandler::recruitCreatures(ObjectInstanceID objid, ObjectInstanceID dstid, CreatureID crid, ui32 cram, si32 fromLvl)
{
	...
	if((!found && complain("Cannot recruit: no such creatures!")) || (cram > VLC->creh->creatures.at(crid)->maxAmount(getPlayer(dst->tempOwner)->resources) && complain("Cannot recruit: lack of resources!")) || (cram <= 0 && complain("Cannot recruit: cram <= 0!")) || (!slot.validSlot() && !warMachine && complain("Cannot recruit: no available slot!")))
	{
		return false;
	}
...
bool CGameHandler::moveArtifact(const ArtifactLocation & al1, const ArtifactLocation & al2)
{
	...
	if((!srcArtifact || dst.slot < GameConstants::BACKPACK_START) && srcArtifact && !srcArtifact->canBePutAt(dst, true))
...
bool CGameHandler::buyArtifact(ObjectInstanceID hid, ArtifactID aid)
{
	...
	else
	{
		...
		if((town->hasBuilt(BuildingID::BLACKSMITH) && town->town->warMachine == aid) || ((town->hasBuilt(BuildingID::BALLISTA_YARD, ETownType::STRONGHOLD)) && aid == ArtifactID::BALLISTA))
		{
...
bool CGameHandler::transformInUndead(const IMarket * market, const CGHeroInstance * hero, SlotID slot)
{
	...
	if(s.hasBonusOfType(Bonus::DRAGON_NATURE) || (s.getCreatureID() == CreatureID::HYDRA) || (s.getCreatureID() == CreatureID::CHAOS_HYDRA))
...
//|| (getHeroCount(player, false) >= GameConstants::MAX_HEROES_PER_PLAYER && complain("Cannot hire hero, only 8 wandering heroes are allowed!")))
	...
	if(t) //tavern in town
	{
		if((!t->hasBuilt(BuildingID::TAVERN) && complain("No tavern!")) || (t->visitingHero && complain("There is visiting hero - no place!")))
		{
			return false;
		}
	}
...
void CGameHandler::updateGateState()
{
	...
	{
		if(!gs->curB->battleGetStackByPos(BattleHex(ESiegeHex::GATE_OUTER), false) && !gs->curB->battleGetStackByPos(BattleHex(ESiegeHex::GATE_INNER), false))
		{
			if(gs->curB->town->subID == ETownType::FORTRESS)
			{
...
bool CGameHandler::makeBattleAction(BattleAction & ba)
{
	...
	case Battle::WALK_AND_ATTACK: //walk or attack
	{
		...
		for(int i = 0; i < totalAttacks; ++i)
		{
			...
			//counterattack
			if(i == 0 && destinationStack && !stack->hasBonusOfType(Bonus::BLOCKS_RETALIATION) && destinationStack->ableToRetaliate() && stack->alive()) //attacker may have died (fire shield)
			{
				BattleAttack bat;
	...
	case Battle::SHOOT:
	{
		...
		if(destinationStack->hasBonusOfType(Bonus::RANGED_RETALIATION) && !stack->hasBonusOfType(Bonus::BLOCKS_RANGED_RETALIATION) && destinationStack->ableToRetaliate() && stack->alive()) //attacker may have died (fire shield)
		{
			BattleAttack bat;
			prepareAttack(bat, destinationStack, stack, 0, stack->position);
		...
		if(destinationStack->alive() && (stack->getCreature()->idNumber == CreatureID::BALLISTA) && (attackingHero->getSecSkillLevel(SecondarySkill::ARTILLERY) >= SecSkillLevel::ADVANCED))
		{
			BattleAttack bat2;
		...
		{
			if(
				stack->alive() && destinationStack->alive() && stack->shots)
			{
				BattleAttack bat;
				bat.flags |= BattleAttack::SHOT;
				prepareAttack(bat, stack, destinationStack, 0, ba.destinationTile);
		...
		for(int g = 0; g < sbi.shots; ++g)
		{
			...
			do //catapult has chance to attack desired target. Othervice - attacks randomly
			{
				if(currentHP.at(attackedPart) != EWallState::DESTROYED && //this part can be hit
				   currentHP.at(attackedPart) != EWallState::NONE && getRandomGenerator().nextInt(99) < getCatapultHitChance(attackedPart, sbi)) //hit is successful
				{
					hitSuccessfull = true;
				}
				else //select new target
				{
					...
					{
						if(currentHP.at(i) != EWallState::DESTROYED && currentHP.at(i) != EWallState::NONE)
			...
			int dmgChance[] =
			{
				sbi.noDmg, sbi.oneDmg, sbi.twoDmg
			}; //dmgChance[i] - chance for doing i dmg when hit is successful
			...
			if(attack.damageDealt > 0 && (attackedPart == EWallPart::KEEP || attackedPart == EWallPart::BOTTOM_TOWER || attackedPart == EWallPart::UPPER_TOWER))
			{
				int posRemove = -1;
				switch(attackedPart)
				{
	...
	while(gs->map->events.size() && gs->map->events.front().firstOccurence + 1 == gs->day)
	{
		...
		for(int player = 0; player < PlayerColor::PLAYER_LIMIT_I; player++)
		{
			...
			if(pinfo //player exists
			   ...
			   && ((ev.computerAffected && !pinfo->human) || (ev.humanAffected && pinfo->human)
			       ))
			{
				//give resources
...
bool CGameHandler::buildBoat(ObjectInstanceID objid)
{
	...
	else if(obj->o->ID == Obj::TOWN && !static_cast<const CGTownInstance *>(obj)->hasBuilt(BuildingID::SHIPYARD))
	{
...
void CGameHandler::handleAfterAttackCasting(const BattleAttack & bat)
{
	...
	if(attacker->hasBonusOfType(Bonus::TRANSMUTATION) && defender->isLiving()) //transmutation mechanics, similar to WoG werewolf ability
	{
		...
		if(defender->getCreature()->idNumber == bonusAdditionalInfo || (bonusAdditionalInfo == -1 && defender->getCreature()->idNumber == attacker->getCreature()->idNumber))
			return;
...
bool CGameHandler::changeStackCount(const StackLocation & sl, TQuantity count, bool absoluteValue /*= false*/)
{
	...
	if((absoluteValue && count < 0)	|| (!absoluteValue && -count > currentCount))
	{
		COMPLAIN_RET("Cannot take more stacks than present!");
	}

	if((currentCount == -count && !absoluteValue) || (!count && absoluteValue))
	{
		eraseStack(sl);
	}
	else
...
void CGameHandler::runBattle()
{
	...
	while(!battleResult.get()) //till the end of the battle ;]
	{
		...
		while(!battleResult.get() && (next = curB.getNextStack()) && next->willMove())
		{
			...
			if(nextStackMorale < 0 && !(NBonus::hasOfType(gs->curB->battleGetFightingHero(0), Bonus::BLOCK_MORALE) || NBonus::hasOfType(gs->curB->battleGetFightingHero(1), Bonus::BLOCK_MORALE)) //checking if gs->curB->heroes have (or don't have) morale blocking bonuses)
			   )
			{
...
void CGameHandler::handleCheatCode(std::string & cheat, PlayerColor player, const CGHeroInstance * hero, const CGTownInstance * town, bool & cheated)
{
	...
	else if(cheat == "vcmiarmenelos")
	{
		...
		{
			if(!town->hasBuilt(build.first)	&& !build.second->Name().empty() && build.first != BuildingID::SHIP)
			{
