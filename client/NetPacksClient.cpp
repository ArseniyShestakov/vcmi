...
void TryMoveHero::applyCl(CClient * cl)
{
	...
	for(auto i = cl->playerint.begin(); i != cl->playerint.end(); i++)
	{
		if(GS(cl)->isVisible(start - int3(1, 0, 0), i->first) || GS(cl)->isVisible(end - int3(1, 0, 0), i->first))
		{
...
void SetHeroesInTown::applyCl(CClient * cl)
{
	...
	for(auto i = cl->playerint.cbegin(); i != cl->playerint.cend(); ++i)
	{
		...
		if(GS(cl)->isVisible(t, i->first) || (hGarr && GS(cl)->isVisible(hGarr, i->first)) || (hVisit && GS(cl)->isVisible(hVisit, i->first)))
		{
