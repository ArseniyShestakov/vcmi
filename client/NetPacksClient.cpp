...
void CALL_IN_PRIVILAGED_INTS(CClient * cl, void (IGameEventsReceiver::*ptr)(Args...), Args && ...args)
{
	for(auto &ger : cl->privilagedGameEventReceivers)
		ger->*ptr(std::forward<Args>(args)...);
}

template<typename ... Args>
void CALL_ONLY_THAT_INTERFACE(CClient * cl, PlayerColor player, void (IGameEventsReceiver::*ptr)(Args...), Args && ...args)
{
	if(vstd::contains(cl->playerint, player))
		cl->playerint[player]->*ptr(std::forward<Args>(args)...);
}

template<typename ... Args>
void INTERFACE_CALL_IF_PRESENT(CClient * cl, PlayerColor player, void (IGameEventsReceiver::*ptr)(Args...), Args && ...args)
{
	CALL_ONLY_THAT_INTERFACE(cl, player, ptr, std::forward<Args>(args)...);
	CALL_IN_PRIVILAGED_INTS(cl, ptr, std::forward<Args>(args)...);
}

template<typename ... Args>
void CALL_ONLY_THAT_BATTLE_INTERFACE(CClient * cl, PlayerColor player, void (IGameEventsReceiver::*ptr)(Args...), Args && ...args)
{
	if(vstd::contains(cl->battleints,player))
		cl->battleints[player]->*ptr(std::forward<Args>(args)...);

	if(cl->additionalBattleInts.count(player))
	{
		for(auto bInt : cl->additionalBattleInts[player])
			bInt->*ptr(std::forward<Args>(args)...);
	}
}

template<typename ... Args>
void BATTLE_INTERFACE_CALL_RECEIVERS(CClient * cl, void (IGameEventsReceiver::*ptr)(Args...), Args && ...args)
{
	for(auto & ber : cl->privilagedBattleEventReceivers)
		ber->*ptr(std::forward<Args>(args)...);
}

template<typename ... Args>
void BATTLE_INTERFACE_CALL_IF_PRESENT(CClient * cl, PlayerColor player, void (IGameEventsReceiver::*ptr)(Args...), Args && ...args)
{
	CALL_ONLY_THAT_INTERFACE(cl, player, ptr, std::forward<Args>(args)...);
	BATTLE_INTERFACE_CALL_RECEIVERS(cl, ptr, std::forward<Args>(args)...);
}

//calls all normal interfaces and privilaged ones, playerints may be updated when iterating over it, so we need a copy
template<typename ... Args>
void CALL_IN_ALL_INTERFACES(CClient * cl, void (IGameEventsReceiver::*ptr)(Args...), Args && ...args)
{
	auto ints = cl->playerint;
	for(auto i = ints.begin(); i != ints.end(); i++)
		CALL_ONLY_THAT_INTERFACE(cl, i->first, ptr, std::forward<Args>(args)...);
}

//calls all normal interfaces and privilaged ones, playerints may be updated when iterating over it, so we need a copy
template<typename ... Args>
void BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(CClient * cl, void (IGameEventsReceiver::*ptr)(Args...), Args && ...args)
{
	CALL_ONLY_THAT_BATTLE_INTERFACE(cl, cl->gs->curB->sides[0].color, ptr, std::forward<Args>(args)...);
	CALL_ONLY_THAT_BATTLE_INTERFACE(cl, cl->gs->curB->sides[1].color, ptr, std::forward<Args>(args)...);
	if(settings["session"]["spectate"].Bool() && !settings["session"]["spectate-skip-battle"].Bool() && LOCPLINT->battleInt)
	{
		CALL_ONLY_THAT_BATTLE_INTERFACE(cl, PlayerColor::SPECTATOR, ptr, std::forward<Args>(args)...);
	}
	BATTLE_INTERFACE_CALL_RECEIVERS(cl, ptr, std::forward<Args>(args)...);
}
...
{
	INTERFACE_CALL_IF_PRESENT(cl, sl.army->tempOwner, &IGameEventsReceiver::stackChangedType, sl, *type);
}
...
{
	INTERFACE_CALL_IF_PRESENT(cl, sl.army->tempOwner, &IGameEventsReceiver::newStackInserted,sl, *sl.getStack());
}
...
{
	CALL_ONLY_THAT_INTERFACE(cl, hero->tempOwner, &IGameEventsReceiver::showTeleportDialog,channel,exits,impassable,queryID);
}

void MapObjectSelectDialog::applyCl(CClient * cl)
{
	CALL_ONLY_THAT_INTERFACE(cl, player, &IGameEventsReceiver::showMapObjectSelectDialog, queryID, icon, title, description, objects);
}

void BattleStart::applyFirstCl(CClient *cl)
{
	//Cannot use the usual macro because curB is not set yet
	CALL_ONLY_THAT_BATTLE_INTERFACE(info->sides[0].color, &IGameEventsReceiver::battleStartBefore, info->sides[0].armyObject, info->sides[1].armyObject,
		info->tile, info->sides[0].hero, info->sides[1].hero);
	CALL_ONLY_THAT_BATTLE_INTERFACE(info->sides[1].color, &IGameEventsReceiver::battleStartBefore, info->sides[0].armyObject, info->sides[1].armyObject,
		info->tile, info->sides[0].hero, info->sides[1].hero);
	CALL_ONLY_THAT_BATTLE_INTERFACE(PlayerColor::SPECTATOR, &IGameEventsReceiver::battleStartBefore, info->sides[0].armyObject, info->sides[1].armyObject,
		info->tile, info->sides[0].hero, info->sides[1].hero);
	BATTLE_INTERFACE_CALL_RECEIVERS(cl, &IGameEventsReceiver::battleStartBefore, info->sides[0].armyObject, info->sides[1].armyObject,
		info->tile, info->sides[0].hero, info->sides[1].hero);
}
...
{
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::battleNewRoundFirst,round);
}

void BattleNextRound::applyCl(CClient *cl)
{
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::battleNewRound,round);
}
...
{
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::battleTriggerEffect, *this);
}

void BattleObstaclePlaced::applyCl(CClient * cl)
{
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::battleObstaclePlaced, *obstacle);
}

void BattleUpdateGateState::applyFirstCl(CClient * cl)
{
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::battleGateStateChanged, state);
}

void BattleResult::applyFirstCl(CClient *cl)
{
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::battleEnd,this);
	cl->battleFinished();
}
...
void BattleStackMoved::applyFirstCl(CClient *cl)
{
	...
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::battleStackMoved,movedStack,tilesToMove,distance);
}
...
void BattleStackAttacked::applyFirstCl(CClient *cl)
{
	...
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::battleStacksAttacked,bsa);
}

void BattleAttack::applyFirstCl(CClient *cl)
{
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::battleAttack,this);
	for (auto & elem : bsa)
	{
...
void StartAction::applyFirstCl(CClient *cl)
{
	...
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::actionStarted, ba);
}

void BattleSpellCast::applyCl(CClient *cl)
{
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::battleSpellCast,this);
}

void SetStackEffect::applyCl(CClient *cl)
{
	//informing about effects
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::battleStacksEffectsSet,*this);
}

void StacksInjured::applyCl(CClient *cl)
{
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::battleStacksAttacked,stacks);
}
...
void StacksHealedOrResurrected::applyCl(CClient * cl)
{
	...
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::battleStacksHealedRes, shiftedHealed, lifeDrain, tentHealing, drainedFrom);
}

void ObstaclesRemoved::applyCl(CClient *cl)
{
	//inform interfaces about removed obstacles
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::battleObstaclesRemoved, obstacles);
}

void CatapultAttack::applyCl(CClient *cl)
{
	//inform interfaces about catapult attack
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::battleCatapultAttacked, *this);
}

void BattleStacksRemoved::applyFirstCl(CClient * cl)
{
	//inform interfaces about removed stacks
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::battleStacksRemoved, *this);
}

void BattleStackAdded::applyCl(CClient *cl)
{
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::battleNewStackAppeared, GS(cl)->curB->stacks.back());
}
...
{
	BATTLE_INTERFACE_CALL_IF_PRESENT_FOR_BOTH_SIDES(&IGameEventsReceiver::actionFinished, *cl->curbaction);
	cl->curbaction.reset();
}

void PackageApplied::applyCl(CClient *cl)
{
	INTERFACE_CALL_IF_PRESENT(cl, player, requestRealized, this);
...
void YourTurn::applyCl(CClient *cl)
{
	...
	CALL_ONLY_THAT_INTERFACE(cl, player, &IGameEventsReceiver::yourTurn);
}
...
{
	CALL_ONLY_THAT_INTERFACE(cl, player, &IGameEventsReceiver::showWorldViewEx, objectPositions);
}
