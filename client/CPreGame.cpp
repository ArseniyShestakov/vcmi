	...
	bg(nullptr), onSelect(OnSelect)
{
	...
	if(tabType != CMenuScreen::campaignList)
	{
		//size filter buttons
		{
			...
			const char * names[] =
			{
				"SCSMBUT.DEF", "SCMDBUT.DEF", "SCLGBUT.DEF", "SCXLBUT.DEF", "SCALBUT.DEF"
			};
			for(int i = 0; i < 5; i++)
		...
		//sort buttons buttons
		{
			int xpos[] =
			{
				23, 55, 88, 121, 306, 339
			};
			const char * names[] =
			{
				"SCBUTT1.DEF", "SCBUTT2.DEF", "SCBUTCP.DEF", "SCBUTT3.DEF", "SCBUTT4.DEF", "SCBUTT5.DEF"
			};
			for(int i = 0; i < 6; i++)
			{
...
CRandomMapTab::CRandomMapTab()
{
	...
	const std::vector<std::string> mapSizeBtns =
	{
		"RANSIZS", "RANSIZM", "RANSIZL", "RANSIZX"
	};
	...
	const std::vector<std::string> waterContentBtns =
	{
		"RANNONE", "RANNORM", "RANISLD"
	};
	...
	const std::vector<std::string> monsterStrengthBtns =
	{
		"RANWEAK", "RANNORM", "RANSTRG"
	};
	...
	difficulty(nullptr)
{
	...
	else
	{
		...
		{
			static const char * difButns[] =
			{
				"GSPBUT3.DEF", "GSPBUT4.DEF", "GSPBUT5.DEF", "GSPBUT6.DEF", "GSPBUT7.DEF"
			};
...
void InfoCard::showTeamsPopup()
{
	...
	for(int i = 0; i < SEL->current->mapHeader->howManyTeams; i++)
	{
		...
		for(int j = 0; j < PlayerColor::PLAYER_LIMIT_I; j++)
			if((SEL->current->mapHeader->players[j].canHumanPlay || SEL->current->mapHeader->players[j].canComputerPlay) && SEL->current->mapHeader->players[j].team == TeamID(i))
				flags.push_back(j);
...
{
	return CGI->heroh->heroes.size() > ID && SEL->sInfo.playerInfos[player].castle == CGI->heroh->heroes[ID]->heroClass->faction && !vstd::contains(usedHeroes, ID) && SEL->current->mapHeader->allowedHeroes[ID];
}
...
void OptionsTab::nextBonus(PlayerColor player, int dir)
{
	...
	if(s.hero == PlayerSettings::NONE && !SEL->current->mapHeader->players[s.color.getNum()].heroesNames.size() && ret == PlayerSettings::ARTIFACT) //no hero - can't be artifact
	{
		if(dir < 0)
...
CMultiMode::CMultiMode()
{
	...
	btns[6] = new CButton(Point(373, 424), "MUBCANC.DEF", CGI->generaltexth->zelp[288], [&] { GH.popIntTotally(this);
			      }, SDLK_ESCAPE);
}
