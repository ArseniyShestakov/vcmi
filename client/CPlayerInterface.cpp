...
void CPlayerInterface::doMoveHero(const CGHeroInstance* h, CGPath path)
{
...

	{
		...
		for(i=path.nodes.size()-1; i>0 && (stillMoveHero.data == CONTINUE_MOVE || curTile->blocked); i--)
		{
			...
			if (priorObject && nextObject && ((priorObject->ID == nextObject->ID) || priorObject->ID == Obj::MONOLITH_ONE_WAY_ENTRANCE && nextObject->ID == Obj::MONOLITH_ONE_WAY_EXIT))
			{
			...
			int3 endpos = path.nodes[i-1].coord;	//(path.nodes[i-1].coord.x, path.nodes[i-1].coord.y, h->pos.z);
