...
void CPlayerInterface::heroMoved(const TryMoveHero & details)
{
	...
	bool directlyAttackingCreature =
		details.attackedFrom && adventureInt->terrain.currentPath //in case if movement has been canceled in the meantime and path was already erased
	...
	if(makingTurn && hero->tempOwner == playerID) //we are moving our hero - we may need to update assigned path
	{
		...
		if(details.result == TryMoveHero::TELEPORTATION)
		{
			if(adventureInt->terrain.currentPath)
			{
				...
				if((nodesIt)->coord == CGHeroInstance::convertPosition(details.start, false) && (nodesIt - 1)->coord == CGHeroInstance::convertPosition(details.end, false))
				{
...
void CPlayerInterface::battleStacksAttacked(const std::vector<BattleStackAttacked> & bsa)
{
	...
	for(auto & elem : bsa)
	{
		...
		StackAttackedInfo to_put =
		{
			defender, elem.damageAmount, elem.killedAmount, attacker, remoteAttack, elem.killed(), elem.willRebirth(), elem.cloneKilled()
		};
...
void CPlayerInterface::showBlockingDialog(const std::string & text, const std::vector<Component> & components, QueryID askID, const int soundID, bool selection, bool cancel)
{
	...
	if(!selection && cancel) //simple yes/no dialog
	{
		...
		showYesNoDialog(text, [ = ] { cb->selectionMade(1, askID);
...
void CPlayerInterface::availableCreaturesChanged(const CGDwelling * town)
{
	...
	else if(GH.listInt.size() && (town->ID == Obj::CREATURE_GENERATOR1 || town->ID == Obj::CREATURE_GENERATOR4 || town->ID == Obj::WAR_MACHINE_FACTORY))
	{
...
{
	EVENT_HANDLER_CALLED_BY_CLIENT;
	if(pa->packType == typeList.getTypeID<MoveHero>() && stillMoveHero.get() == DURING_MOVE && destinationTeleport == ObjectInstanceID())
		stillMoveHero.setn(CONTINUE_MOVE);

	if(destinationTeleport != ObjectInstanceID() && pa->packType == typeList.getTypeID<QueryReply>() && stillMoveHero.get() == DURING_MOVE)
...
void CPlayerInterface::newObject(const CGObjectInstance * obj)
{
	...
	if(obj->ID == Obj::BOAT	&& LOCPLINT->castleInt && obj->pos - obj->getVisitableOffset() == LOCPLINT->castleInt->town->bestLocation())
	{
		CCS->soundh->playSound(soundBase::newBuilding);
...
void CPlayerInterface::update()
{
	...
	if(adventureInt && GH.topInt() == adventureInt && (!adventureInt->selection && !settings["session"]["spectate"].Bool()))
	{
		return;
	}
...
{
	showYesNoDialog(CGI->generaltexth->allTexts[68], [this] { sendCustomEvent(RETURN_TO_MENU_LOAD);
			}, 0, false);
}
