...
void CMapHandler::initObjectRects()
{
	...
	for(auto & elem : map->objects)
	{
		...
		if(!obj	|| (obj->ID == Obj::HERO && static_cast<const CGHeroInstance *>(obj)->inTownGarrison) //garrisoned hero
		...
		for(int fx = 0; fx < obj->getWidth(); ++fx)
		{
			for(int fy = 0; fy < obj->getHeight(); ++fy)
			{
				...
				if(map->isInTheMap(currTile) && //within map
				   ...
				   cr.y + cr.h > 0 && obj->coveringAt(currTile.x, currTile.y) //object is visible here
				   )
				{
...
ui8 CMapHandler::CMapBlitter::getHeroFrameGroup(ui8 dir, bool isMoving) const
{
	if(isMoving)
	{
		static const ui8 frame[] =
		{
			0xff, 10, 5, 6, 7, 8, 9, 12, 11
		};
		return frame[dir];
	}
	else //if(isMoving)
