...
bool CGarrisonSlot::split()
{
	...
	if(upg != selection->upg) //not splitting within same army
	{
		...
		if(getObj()->stacksCount() == 1	&& selection->creature == creature && getObj()->needsLastStack())
		{
			minRight = 1;
		}
	}
...
void CGarrisonSlot::clickLeft(tribool down, bool previousState)
{
	if(down)
	{
		...
		else
		{
			...
			if(selectedObj->stacksCount() == 1 && owner->getSelection()->upg != upg	&& dynamic_cast<const CGHeroInstance *>(selectedObj))
			{
	...
	CWindowObject(PLAYER_COLORED, "GARRISON")
{
	...
	quit = new CButton(Point(399, 314), "IOK6432.DEF", CButton::tooltip(CGI->generaltexth->tcommands[8], ""), [&] { close();
			   }, SDLK_RETURN);
