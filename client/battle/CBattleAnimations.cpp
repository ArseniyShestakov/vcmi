	...
	dest(_dest), attackedStack(defender), attackingStack(attacker)
{
	...
	bool isCatapultAttack = attackingStack->hasBonusOfType(Bonus::CATAPULT)	&& owner->getCurrentPlayerInterface()->cb->battleHexToWallPart(_dest) >= 0;
...
bool CMovementAnimation::init()
{
	...
	if(owner->creAnims[stack->ID]->framesInGroup(CCreatureAnim::MOVING) == 0 || stack->hasBonus(Selector::typeSubtype(Bonus::FLYING, 1)))
	{
...
bool CMovementEndAnimation::init()
{
	...
	if(!stack || myAnim->framesInGroup(CCreatureAnim::MOVE_END) == 0 || myAnim->isDead())
	{
		endAnim();
