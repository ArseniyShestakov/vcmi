...
static void transformPalette(SDL_Surface * surf, double rCor, double gCor, double bCor)
{
	...
	{
		if((colorsToChange + g)->b != 132 && (colorsToChange + g)->g != 231 && (colorsToChange + g)->r != 255) //it's not yellow border
		{
...
//blitAt(menu, pos.x, 556 + pos.y);
	...
	if(tacticsMode)
	{
		btactNext = new CButton(Point(213, 560), "icm011.def", std::make_pair("", ""), [&] { bTacticNextStack(nullptr);
					}, SDLK_SPACE);
		btactEnd = new CButton(Point(419, 560), "icm012.def", std::make_pair("", ""), [&] { bEndTacticPhase();
				       }, SDLK_RETURN);
...
void CBattleInterface::setBattleCursor(const int myNumber)
{
	...
	if(!vstd::contains_if(sectorCursor, [](int sc)
	{
		return sc != -1;
	}))
	{
...
void CBattleInterface::bSurrenderf()
{
	...
	if(cost >= 0)
	{
		...
		curInt->showYesNoDialog(surrenderMessage, [this] { reallySurrender();
					}, 0, false);
	}
}
...
void CBattleInterface::endAction(const BattleAction * action)
{
	...
	if(stack && action->actionType == Battle::WALK && !creAnims[action->stackNumber]->isIdle()) //walk or walk & attack
	{
	...
	for(const CStack * s : stacks)
	{
		if(s && creDir[s->ID] != (s->side == BattleSide::ATTACKER) && s->alive() && creAnims[s->ID]->isIdle())
		{
...
void CBattleInterface::startAction(const BattleAction * action)
{
	...
	if(action->actionType == Battle::WALK || (action->actionType == Battle::WALK_AND_ATTACK && action->destinationTile != stack->position))
	{
		assert(stack);
		moveStarted = true;
...
void CBattleInterface::handleHex(BattleHex myNumber, int eventType)
{
	...
	for(PossibleActions action : possibleActions)
	{
		...
		case RISE_DEMONS:
			...
			{
				if(!(shere->hasBonusOfType(Bonus::UNDEAD) || shere->hasBonusOfType(Bonus::NON_LIVING) || vstd::contains(shere->state, EBattleStackState::SUMMONED) || vstd::contains(shere->state, EBattleStackState::CLONED) || shere->hasBonusOfType(Bonus::SIEGE_WEAPON)
				     ))
					legalAction = true;
			}
			break;
	...
	if(currentAction > INVALID)
	{
		...
		case SHOOT:
		{
			...
			realizeAction = [ = ] {
				giveCommand(Battle::SHOOT, myNumber, activeStack->ID);
			};
			std::string estDmgText = formatDmgRange(curInt->cb->battleEstimateDamage(CRandomGenerator::getDefault(), sactive, shere)); //calculating estimated dmg
		...
		case HEAL:
			...
			realizeAction = [ = ] {
				giveCommand(Battle::STACK_HEAL, myNumber, activeStack->ID);
			}; //command healing
			break;
		case RISE_DEMONS:
		...
		case CATAPULT:
			...
			realizeAction = [ = ] {
				giveCommand(Battle::CATAPULT, myNumber, activeStack->ID);
			};
			break;
		case CREATURE_INFO:
		{
			...
			realizeAction = [ = ] {
				GH.pushInt(new CStackWindow(shere, false));
			};
			break;
		}
		}
...
void CBattleInterface::SiegeHelper::printPartOfWall(SDL_Surface * to, int what)
{
	...
	if(town->town->faction->index == ETownType::TOWER && (what == SiegeHelper::MOAT || what == SiegeHelper::BACKGROUND_MOAT))
	...
	for(int b = 0; b < GameConstants::BFIELD_SIZE; ++b)
	{
		if(bfield[b]->strictHovered && bfield[b]->hovered)
		{
			...
			if(settings["battle"]["mouseShadow"].Bool())
			{
				...
				{
					if(currentlyHoveredHex.getX() != 0 && currentlyHoveredHex.getX() != GameConstants::BFIELD_WIDTH - 1)
...
void CBattleInterface::showProjectiles(SDL_Surface * to)
{
	...
	for(auto it = projectiles.begin(); it != projectiles.end(); ++it)
	{
		...
		if(!it->shotDone)
		{
			...
			if(creAnims[it->stackID]->getCurrentFrame() >= it->animStartDelay || creAnims[it->stackID]->isShooting() == false)
			{
...
BattleObjectsByHex CBattleInterface::sortObjectsByHex()
{
	...
	//Sort obstacles
	{
		...
		{
			if(obstacle->obstacleType != CObstacleInstance::ABSOLUTE_OBSTACLE && obstacle->obstacleType != CObstacleInstance::MOAT)
			{
