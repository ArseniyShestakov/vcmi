...
void CCastleBuildings::enterMagesGuild()
{
	...
	if(hero && !hero->hasSpellbook()) //hero doesn't have spellbok
	{
		...
		{
			CFunctionList<void()> onYes = [this] {
				openMagesGuild();
			};
			CFunctionList<void()> onNo = onYes;
			onYes += [hero] {
				LOCPLINT->cb->buyArtifact(hero, ArtifactID::SPELLBOOK);
			};
...
{
	if(town->visitingHero && town->visitingHero->hasArt(2) && !vstd::contains(town->builtBuildings, BuildingID::GRAIL)) //hero has grail, but town does not have it
	{
	...
	town(Town)
{
	...
	exit = new CButton(Point(744, 544), "TSBTNS", CButton::tooltip(CGI->generaltexth->tcommands[8]), [&] {close();
			   }, SDLK_RETURN);
	...
	town(Town)
{
	...
	exit = new CButton(Point(748, 556), "TPMAGE1.DEF", CButton::tooltip(CGI->generaltexth->hcommands[8]), [&] {close();
			   }, SDLK_RETURN);
	exit->assignedKeys.insert(SDLK_ESCAPE);
	...
	building(Building)
{
	...
	{ //normal window
		...
		CButton * buy = new CButton(Point(45, 446), "IBUY30", CButton::tooltip(tooltipYes), [&] { buyFunc();
					    }, SDLK_RETURN);
		buy->borderColor = Colors::METALLIC_GOLD;
		buy->block(state != 7 || LOCPLINT->playerID != town->tempOwner);

		CButton * cancel = new CButton(Point(290, 445), "ICANCEL", CButton::tooltip(tooltipNo), [&] { close();
					       }, SDLK_ESCAPE);
	...
	CWindowObject(PLAYER_COLORED | BORDERED, getBgName(town))
{
	...
	exit = new CButton(Point(748, 556), "TPMAGE1", CButton::tooltip(text), [&] { close();
			   }, SDLK_RETURN);
	exit->assignedKeys.insert(SDLK_ESCAPE);
	...
	CWindowObject(BORDERED, imagem)
{
	...
	exit = new CButton(Point(748, 556), "TPMAGE1.DEF", CButton::tooltip(CGI->generaltexth->allTexts[593]), [&] { close();
			   }, SDLK_RETURN);
	exit->assignedKeys.insert(SDLK_ESCAPE);
	...
	CWindowObject(PLAYER_COLORED, "TPSMITH")
{
	...
	buy = new CButton(Point(42, 312), "IBUY30.DEF", CButton::tooltip(text), [&] { close();
			  }, SDLK_RETURN);

	text = boost::str(boost::format(CGI->generaltexth->allTexts[596]) % creature->nameSing);
	cancel = new CButton(Point(224, 312), "ICANCEL.DEF", CButton::tooltip(text), [&] { close();
			     }, SDLK_ESCAPE);
