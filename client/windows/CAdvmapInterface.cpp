...
void CAdvMapInt::handleMapScrollingUpdate()
{
	...
	if((animValHitCount % (4 / scrollSpeed)) == 0 && ((GH.topInt() == this) || isCtrlKeyDown()))
	{
...
{
	state = INGAME;
	if(LOCPLINT->cb->getCurrentPlayer() == LOCPLINT->playerID || settings["session"]["spectate"].Bool())
	{
	...
	CWindowObject(PLAYER_COLORED, "ADVOPTS")
{
	...
	viewWorld = new CButton(Point(24, 23), "ADVVIEW.DEF", CButton::tooltip(), [&] { close();
				}, SDLK_v);
	...
	scenInfo = new CButton(Point(24, 198), "ADVINFO.DEF", CButton::tooltip(), [&] { close();
			       }, SDLK_i);
	scenInfo->addCallback(CAdventureOptions::showScenarioInfo);

	puzzle = new CButton(Point(24, 81), "ADVPUZ.DEF", CButton::tooltip(), [&] { close();
			     }, SDLK_p);
	puzzle->addCallback(std::bind(&CPlayerInterface::showPuzzleMap, LOCPLINT));

	dig = new CButton(Point(24, 139), "ADVDIG.DEF", CButton::tooltip(), [&] { close();
			  }, SDLK_d);
