	...
	onFullscreenChanged(settings.listen["video"]["fullscreen"])
{
	...
	//setting up buttons
	load = new CButton(Point(246, 298), "SOLOAD.DEF", CGI->generaltexth->zelp[321], [&] { bloadf();
			   }, SDLK_l);
	load->setImageOrder(1, 0, 2, 3);

	save = new CButton(Point(357, 298), "SOSAVE.DEF", CGI->generaltexth->zelp[322], [&] { bsavef();
			   }, SDLK_s);
	save->setImageOrder(1, 0, 2, 3);

	restart = new CButton(Point(246, 357), "SORSTRT", CGI->generaltexth->zelp[323], [&] { brestartf();
			      }, SDLK_r);
	restart->setImageOrder(1, 0, 2, 3);

	mainMenu = new CButton(Point(357, 357), "SOMAIN.DEF", CGI->generaltexth->zelp[320], [&] { bmainmenuf();
			       }, SDLK_m);
	mainMenu->setImageOrder(1, 0, 2, 3);

	quitGame = new CButton(Point(246, 415), "soquit.def", CGI->generaltexth->zelp[324], [&] { bquitf();
			       }, SDLK_q);
	quitGame->setImageOrder(1, 0, 2, 3);

	backToMap = new CButton(Point(357, 415), "soretrn.def", CGI->generaltexth->zelp[325], [&] { breturnf();
				}, SDLK_RETURN);
	...
	onFullscreenChanged([&](const JsonNode & newState)
	{
		fullscreen->setSelected(newState.Bool());
	});
...
{
	LOCPLINT->showYesNoDialog(CGI->generaltexth->allTexts[578], [this] { closeAndPushEvent(SDL_USEREVENT, FORCE_QUIT);
				  }, 0);
}
...
{
	LOCPLINT->showYesNoDialog(CGI->generaltexth->allTexts[578], [this] { closeAndPushEvent(SDL_USEREVENT, RETURN_TO_MAIN_MENU);
				  }, 0);
}
...
{
	LOCPLINT->showYesNoDialog(CGI->generaltexth->allTexts[67], [this] { closeAndPushEvent(SDL_USEREVENT, RESTART_GAME);
				  }, 0);
}
	...
	town(_town)
{
	...
	all = new CButton(Point(146, 416), "ALTARMY.DEF", CGI->generaltexth->zelp[590], [&] { addAll();
			  }, SDLK_a);
	convert = new CButton(Point(269, 416), "ALTSACR.DEF", CGI->generaltexth->zelp[591], [&] { makeDeal();
			      }, SDLK_RETURN);
	cancel = new CButton(Point(392, 416), "ICANCEL.DEF", CGI->generaltexth->zelp[592], [&] { close();
			     }, SDLK_ESCAPE);
	...
	market(_market)
{
	...
	cancel = new CButton(Point(200, 313), "IOKAY.DEF", CGI->generaltexth->zelp[632], [&] { close();
			     }, SDLK_RETURN);
	...
	parent(PARENT)
{
	...
	confirm = new CButton(Point(148, 299), "IBY6432.DEF", CButton::tooltip(hoverText, text), [ = ] {makeDeal(SKILL);
			      }, SDLK_RETURN);
	confirm->block(!available);

	cancel = new CButton(Point(252, 299), "ICANCEL.DEF", CGI->generaltexth->zelp[631], [&] { close();
			     }, SDLK_ESCAPE);
	bar = new CGStatusBar(new CPicture(*background, Rect(8, pos.h - 26, pos.w - 16, 19), 8, pos.h - 26));
}
	...
	hero(visitor)
{
	...
	{
		upgrade[i] = new CButton(Point(107 + i * 76, 171), "", CButton::tooltip(getTextForSlot(SlotID(i))), [ = ] { makeDeal(SlotID(i));
					 }, SDLK_1 + i);
	...
	upgradeAll = new CButton(Point(30, 231), "", CButton::tooltip(CGI->generaltexth->allTexts[432]), [&] { makeDeal(SlotID(slotsCount));
				 }, SDLK_0);
	...
	owner(_owner)
{
	...
	exitb = new CButton(Point(748, 556), "TPMAGE1", CButton::tooltip(CGI->generaltexth->allTexts[600]), [&] { close();
			    }, SDLK_RETURN);
	...
	static const std::string colorToBox[] =
	{
		"PRRED.BMP", "PRBLUE.BMP", "PRTAN.BMP", "PRGREEN.BMP", "PRORANGE.BMP", "PRPURPLE.BMP", "PRTEAL.BMP", "PRROSE.bmp"
	};
