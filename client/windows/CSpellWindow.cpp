...
void CSpellWindow::computeSpellsPerArea()
{
	...
	{
		if(spell->isCombatSpell() ^ !battleSpellsOnly && ((selectedTab == 4) || spell->school.at((ESpellSchool)selectedTab)))
		{
...
void CSpellWindow::keyPressed(const SDL_KeyboardEvent & key)
{
	...
	if(key.state == SDL_PRESSED)
	{
		...
		if(myInt->altPressed())
		{
			...
			static const SDL_Keycode spellSelectors[] =
			{
...
void CSpellWindow::SpellArea::clickLeft(tribool down, bool previousState)
{
	if(mySpell && !down)
	{
		...
		if((mySpell->isCombatSpell() && !owner->myInt->battleInt) || (mySpell->isAdventureSpell() && (owner->myInt->battleInt || owner->myInt->castleInt)))
		{
		...
		if(mySpell->isCombatSpell() && owner->myInt->battleInt) //if battle window is open
		{
			...
			case ESpellCastProblem::ANOTHER_ELEMENTAL_SUMMONED:
			{
				...
				const TStacks stacks = owner->myInt->cb->battleGetStacksIf([player](const CStack * s)
					{
						return s->owner == player && vstd::contains(s->state, EBattleStackState::SUMMONED) && !vstd::contains(s->state, EBattleStackState::CLONED);
					});
...
void CSpellWindow::SpellArea::setSpell(const CSpell * spell)
{
	...
	if(mySpell)
	{
		...
		{
			static const SDL_Color unavailableSpell =
			{
				239, 189, 33, 0
			};
